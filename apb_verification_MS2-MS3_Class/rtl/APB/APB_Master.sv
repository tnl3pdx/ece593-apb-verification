...
always @(posedge i_pclk or negedge i_prstn)
  if (!i_prstn) begin
    ...
    ACCESS: begin
    o_transfer_status<=(i_pready) ? i_pslverr : 1'b0;     //Updating the transfer status according to the pslverr signal generated by the slave
    if (i_pready) begin                                               //The 'penable' signal goes low when 'pready' is logic high 
      o_penable<=1'b0;
      o_psel<='0;
      if (!i_command) begin                                                //Update the output date at the end of a read command
        o_data_out<=i_prdata;
      end else begin
        o_data_out<='0;
      end
    ...
    // Default: In case of an invalid state, reset all outputs to default values
    default: 
    begin
      o_paddr<='0; 
      o_psel<='0;	  
