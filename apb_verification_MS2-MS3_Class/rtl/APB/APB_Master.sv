...
always @(posedge i_pclk or negedge i_prstn)
  if (!i_prstn) begin
    ...
    if (i_pready) begin                                               //The 'penable' signal goes low when 'pready' is logic high 
      o_penable<=1'b0;
      if (i_start) begin
        // Start a new (back-to-back) transfer: sample address/command/data and assert new psel
        o_paddr<=i_addr_in;
        o_pwrite<=i_command;
        case (i_addr_in[(ADDR_WIDTH-1)-:ADDR_MSB_len])
        ADDR_SLAVE_0: o_psel<=3'b001;
        ...
        // No new transfer requested: deassert psel
        o_psel<='0;
      end
...
always @(posedge i_pclk or negedge i_prstn)
 ...
 //assign o_valid = (state == ACCESS) && o_psel && o_penable && i_pready && ~o_pwrite; // Alternative valid generation logic
 
 // Ready is high when bus is idle, and also on ACCESS completion when a
 // back-to-back transfer is accepted (ACCESS -> SETUP while i_start is high).
 assign o_ready = (state==IDLE)
               || (state==ACCESS && next_state==SETUP);
endmodule
